...
Functional programming is a good habit. It keeps your code easy to manage, and saves you from sneaky bugs. But before we get there, 
let's look at an imperative approach to programming to highlight where you may have issues.

In English (and many other languages), the imperative tense is used to give commands. Similarly, an imperative style in programming 
is one that gives the computer a set of statements to perform a task.

Often the statements change the state of the program, like updating global variables. A classic example is writing a for loop that 
gives exact directions to iterate over the indices of an array.
...
const Window = function(tabs) {
  ...
  Window.prototype.tabClose = function(index) {
    ...
    const tabsBeforeIndex = this.tabs.slice(0, index); // Get the tabs before the tab // splice => slice
    const tabsAfterIndex = this.tabs.slice(index + 1); // Get the tabs after the tab // splice => slice
